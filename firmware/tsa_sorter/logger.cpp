...
void Logger::begin() {
    ...
    delay(100);
    _mode = LogMode::HUMAN;
    Serial.println("[boot] logger ready");
}
...
void Logger::senseVerbose(uint8_t brickNum, const SenseResult& s) {
    ...
    Serial.printf("[sense] brick=%u  result=%s\n",
                  brickNum, sensors::categoryName(s.category));
    Serial.printf("[sense] size:  lenA=%.2fmm lenB=%.2fmm lenAvg=%.2fmm  thresh=%.2fmm  -> %s  valid=%s\n",
                  s.measuredLengthAmm, s.measuredLengthBmm, s.measuredLengthAvgMm,
                  sizeThresh, s.isTwoByThree ? "2x3" : "2x2", s.sizeValid ? "y" : "N");
    Serial.printf("[sense] color: ratio=%.4f  thresh=%.4f  -> %s  samples=%u\n",
                  s.redRatio, colorThresh, s.redRatio >= colorThresh ? "RED" : "blue",
                  s.sampleCount);
    Serial.printf("[sense] raw:   R=%.0f B=%.0f C=%.0f  net R=%.0f B=%.0f\n",
                  s.rawRedHz, s.rawBlueHz, s.rawClearHz, s.redNetHz, s.blueNetHz);
    Serial.printf("[sense] belt:  lead=%.1f trail=%.1f avg=%.1f mm/s  speedSkew=%.1f%% lenSkew=%.1f%%\n",
                  s.beltSpeedLeadMms, s.beltSpeedTrailMms, s.beltSpeedAvgMms,
                  s.speedSkewPct, s.lengthSkewPct);
}

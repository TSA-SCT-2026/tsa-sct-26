...
"${OPENSCAD_CMD[@]}" -o "$OUT_DIR/_validate_params.stl" -D 'build_target="validate"' "$SRC_DIR/drive_pulley.scad"
rm -f "$OUT_DIR/_validate_params.stl"

echo "[2/7] dimension report"
"${OPENSCAD_CMD[@]}" -o "$OUT_DIR/_dimension_report.stl" -D 'build_target="report"' "$SRC_DIR/drive_pulley.scad"
rm -f "$OUT_DIR/_dimension_report.stl"

echo "[3/7] render drive pulley"
"${OPENSCAD_CMD[@]}" -o "$OUT_DIR/drive_pulley_proto_v1.stl" -D 'build_target="model"' "$SRC_DIR/drive_pulley.scad"
...
"${OPENSCAD_CMD[@]}" -o "$OUT_DIR/coupon_d_bore_fit_v1.stl" -D 'build_target="dbore"' "$SRC_DIR/roller_coupons.scad"

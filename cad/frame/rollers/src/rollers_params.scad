...
function default_params() = [
  ...
  kv("provisional_d_bore_round", 5.20),
  kv("provisional_d_bore_to_flat", 4.50),
  kv("provisional_split_clamp_slot", 1.8),
  kv("provisional_clamp_screw_axis_x", 4.8),
  kv("provisional_clamp_screw_clear_d", 3.2),
  kv("provisional_clamp_screw_head_d", 5.8),
  kv("provisional_clamp_screw_head_depth", 3.0),
  kv("provisional_clamp_nut_flat", 5.7),
  kv("provisional_clamp_nut_thickness", 2.6),
...
module validate(params = default_params()) {
  ...
  assert(pget(params, "provisional_clamp_screw_axis_x") > (pget(params, "provisional_d_bore_round") / 2),
    "Clamp screw axis must stay outside the shaft bore");
  assert(pget(params, "provisional_clamp_screw_axis_x") < ((pget(params, "provisional_drive_hub_od") / 2) - (pget(params, "provisional_clamp_screw_clear_d") / 2)),
  ...
  if (pget(params, "provisional_d_bore_round") == 5.20)
...
module dimension_report(params = default_params()) {
  ...
  echo(str("PROVISIONAL_ESTIMATE clamp_screw_axis_x=", pget(params, "provisional_clamp_screw_axis_x")));
